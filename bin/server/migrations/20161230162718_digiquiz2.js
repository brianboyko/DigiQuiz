...
exports.up = function (knex, Promise) {
  return Promise.all([knex.schema.createTable('USERS', function (table) {
    ...
    table.integer('created_by').references('uid').inTable('USERS');
    ...
    table.integer('deck').references('id').inTable('DECKS');
    table.integer('created_by').references('uid').inTable('USERS');
    ...
    table.integer('player').references('uid').inTable('USERS');
    ...
    table.increments('id').primary();
    table.integer('question').references('id').inTable('QUESTIONS');
    table.integer('keyword').references('id').inTable('KEYWORDS');
  }), knex.schema.createTable('USERS_DECKS', function (table) {
    table.increments('id').primary();
    table.integer('user').references('uid').inTable('USERS');
    table.integer('deck').references('id').inTable('DECKS');
